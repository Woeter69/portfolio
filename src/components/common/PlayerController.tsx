...
export default function PlayerController({ onUnlock, showButton = true }: PlayerControllerProps) {
  ...
  // Unlock pointer lock when focused on a prop, and re-lock automatically when exiting prop focus
  useEffect(() => {
    if (focusedProp !== 'none') {
  ...
  return (
    <group>
      <PointerLockControls 
        ...
        onUnlock={() => {
          if (useInteractionStore.getState().focusedProp === 'none') {
